...
const SuperAdminDashboard = () => {
  ...
  const { systemStatus, refreshStats } = useSystemStatus();
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card className="border-0 shadow-xl bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${systemStatus.isEmergencyMode ? 'bg-red-500 animate-pulse' : 'bg-green-500 animate-pulse'}`}></div>
                    <Shield className="h-4 w-4 text-blue-600" />
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold text-blue-900 dark:text-blue-100">
                    {systemStatus.isEmergencyMode ? 'Emergency' : 'Optimal'}
                  </div>
                  <p className="text-xs text-blue-700 dark:text-blue-300">
                    {systemStatus.failedLogins > 0 ? `${systemStatus.failedLogins} failed logins today` : 'All systems operational'}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="border-0 shadow-xl bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900 dark:to-green-800">
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-green-900 dark:text-green-100">{systemStatus.activeUsers}</div>
                  <p className="text-xs text-green-700 dark:text-green-300">
                    {systemStatus.recentLogins > 0 ? `${systemStatus.recentLogins} logins today` : 'No recent activity'}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="border-0 shadow-xl bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900 dark:to-purple-800">
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-purple-900 dark:text-purple-100">{systemStatus.totalOrganizations}</div>
                  <p className="text-xs text-purple-700 dark:text-purple-300">
                    {systemStatus.totalOrganizations > 0 ? `${Math.round(systemStatus.activeUsers / Math.max(systemStatus.totalOrganizations, 1))} avg users/org` : 'No organizations yet'}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="border-0 shadow-xl bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900 dark:to-orange-800">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${systemStatus.securityScore >= 90 ? 'bg-green-500' : systemStatus.securityScore >= 70 ? 'bg-yellow-500' : 'bg-red-500'} animate-pulse`}></div>
                    <Shield className="h-4 w-4 text-orange-600" />
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold text-orange-900 dark:text-orange-100">{systemStatus.securityScore}%</div>
                  <p className="text-xs text-orange-700 dark:text-orange-300">
                    {systemStatus.securityScore >= 90 ? 'Excellent security' : systemStatus.securityScore >= 70 ? 'Good security' : 'Needs attention'}
                  </p>
                </CardContent>
              </Card>
            </div>
